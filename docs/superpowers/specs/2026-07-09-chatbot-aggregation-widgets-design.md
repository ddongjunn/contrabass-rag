...
- 실시간 스트리밍 / 인터랙티브 시계열 (정적 스냅샷 스파크라인은 포함)
- LLM 호출 추가 (위젯·연관질문 칩 모두 LLM 0회 — 기존 조회 결과·추출값의 재표현/규칙 생성)
- 문서(DOC) 경로 변경 (기존 평문 + 출처칩 유지)

## 3. 범위 (Phase 1 위젯 세트 확정)

> **결정(2026-07-09)**: 확장 위젯 중 `inventory_table`은 **제외**, 나머지(스파크라인·프로젝트별
> 바·상태 도넛·쿼터 게이지·임계 배너)와 **연관질문 칩**을 **Phase 1에 포함**한다.
> **여파**: Phase 1이 초안의 "재표현만(집계 변경 0)"에서 **새 집계 쿼리 포함**으로 확장된다.
> 이를 감안해 Phase 1을 두 티어로 나눠 순차 진행한다.

### Phase 1 (이번)
| 위젯 | 티어 | 소스/집계 |
|---|---|---|
| `metric_rank` | 1a · 재표현 | 기존 TopN(`MetricSample`) |
| `inventory_count` | 1a · 재표현 | 기존 COUNT(`InventoryResult`) |
| 연관질문 칩(`followups`) | 1a · 규칙 | `ResourceQuery`+top 결과, LLM 0회 (§7.1) |
| `metric_rank` 행 스파크라인 | 1b · 신규 | Prometheus **range 쿼리** 신규 |
| `project_usage_bar` | 1b · 신규 | PromQL **`by(project)`** 신규 |
| `status_donut` | 1b · 신규 | cb_common **group by status** 신규 |
| `quota_gauge` | 1b · 신규 | **쿼터 데이터 소스** 신규 |
| `threshold_banner` | 1b · 신규 | PromQL **임계 초과 count** 신규 |
| empty/error 상태 | 1a | — |

- **1a(재표현·규칙)**: 기존 조회 결과 재사용 + 규칙 생성. 라우팅/집계 변경 0. 먼저 착수.
- **1b(신규 집계)**: 위 표대로 **새 쿼리/소스**가 필요. BE-B의 집계 작업이 늘어난다(§5.4).

### Phase 2
| 포함 | 이유 |
|---|---|
| `resource_dashboard` (요약 라우트·크로스소스 집계) | 새 의도 분류 + 고정 PromQL 세트 + cb_common·Prometheus 결합. 1b 위젯들을 조합한 종합 뷰 |

### Backlog (제외/보류)
- `inventory_table`(LIST 표) — **이번 제외**(결정). 필요 시 재검토.
- heatmap, 실시간 time-range 셀렉터 — 의도적 제외(§8).
...
data class ChatResponse(
    ...
    val followups: List<String> = emptyList(),  // 연관질문 칩(규칙 생성, LLM 0회). §7.1
)
```

`answer`(평문)는 **항상** 함께 반환한다. 웹 위젯은 시각적 보강일 뿐, 평문이 진실의 원본이자
Slack·스크린리더 폴백이다. `followups`도 기본 빈 배열 → Slack·기존 클라이언트 무영향(웹만 칩 렌더).
...
// metric_rank — Prometheus TopN (ResourceQuery + List<MetricSample>)
{
  ...
  "rows": [
    { "instanceName": "web-prod-07", "projectName": "service-prod",
      ...
      "spark": [58, 61, 60, 67, 72, 80, 85, 91] }             // 1b: range 쿼리 시계열(옵션)
  ]
}
...
// project_usage_bar (1b) — PromQL by(project)
{ "type": "project_usage_bar", "metric": "CPU", "unit": "%",
  "rows": [ { "projectName": "service-prod", "value": 78.4, "display": "78.4%", "severity": "warn" } ] }

// status_donut (1b) — cb_common group by status
...
// quota_gauge (1b) — 쿼터 데이터
{ "type": "quota_gauge",
  "items": [ { "resource": "vCPU", "used": 820, "quota": 1000, "ratio": 0.82, "display": "820 / 1000", "severity": "warn" } ] }

// threshold_banner (1b) — PromQL 임계 초과 count
...
### 5.4 Phase 1b 신규 집계 (결정 여파)

1a와 달리 아래 위젯은 **새 쿼리/소스**가 필요하다. BE-B 작업량이 그만큼 는다.
각 위젯은 여전히 **LLM 0회**(집계는 PromQL/SQL, 위젯은 순수 변환).

| 위젯 | 신규 집계 | 위치(제안) |
|---|---|---|
| `metric_rank` 스파크라인 | Prometheus **range 쿼리**(`/api/v1/query_range`) → 행별 시계열 | `PrometheusClient` 확장 |
| `project_usage_bar` | PromQL **`avg by(project)`** | `PromQlBuilder`/`ResourceService` |
| `status_donut` | cb_common **`GROUP BY status`** 카운트 | `InventoryRepository`/`InventorySql` |
| `quota_gauge` | 쿼터 사용량/한도 조회(쿼터 테이블·API 소스 확인 필요) | 신규 조회(소스 확정 후) |
| `threshold_banner` | PromQL **`count(metric > threshold)`** + 초과 인스턴스명 | `ResourceService` + severity config(§5.1) |

> **선행 확인**: `quota_gauge`는 cb_common에 쿼터 데이터가 있는지부터 확인해야 한다(없으면
> 별도 소스·API 필요 → 1b 중 가장 불확실). BE-B가 착수 전 스파이크로 검증.
...
- **레이아웃(결정)**: 챗봇 캡션(문장)과 위젯은 **하나의 카드로 합쳐** 렌더(분리 X). 카드 상단
  캡션 → 구분선 → 위젯. 어시스턴트 답변 **앞에 챗봇 프로필 아바타**를 붙인다. (프로토타입 반영)
- `followups`(연관질문 칩)는 카드 아래 렌더, 클릭 시 해당 질문 재전송.

브랜드/레이아웃 참조: 정적 프로토타입 `docs/prototype/chatbot-widgets/index.html`(대화 데모),
`gallery.html`(위젯 후보 갤러리). 보는 법·주의는 해당 폴더 `README.md` 참고.

### 7.1 연관질문 칩(`followups`) 생성 규칙 (Phase 1, LLM 0회)

이미 추출한 `ResourceQuery`(metric/project/instanceName)와 top 결과로 **규칙 생성**한다.
LLM 추가 호출 없음(루트 불변식·비용 방어선과 정합).
...
| 인벤토리 확장 | `InventoryKind` | "스냅샷은 몇 개야?" |

- 백엔드가 `ChatResponse.followups`로 최대 3개 반환. RESOURCE·INVENTORY 경로에만.
- (향후 옵션) DOC 경로는 생성 LLM을 이미 부르므로, 그때 후속질문을 함께 뽑는 것도 가능(추가 비용 최소). 이번 범위 아님.

## 8. 확장 위젯 후보 (결정 완료)

**Phase 1b로 채택**(2026-07-09) — 상세는 §3·§5.4:
임계 초과 배너 · 프로젝트별 사용량 바 · 상태 분포 도넛 · 쿼터/용량 게이지 · rank 행 스파크라인.

**제외/보류**:
- `inventory_table`(LIST 표) — 이번 제외. 필요 시 재검토.
- heatmap(노드×지표, 과함), 실시간 time-range 셀렉터(스냅샷 비목표와 충돌) — 의도적 제외.
...
data class QuotaItem(val resource: String, val used: Double, val quota: Double, val ratio: Double, val display: String, val severity: Severity?)
