...
> **현재 상태 (2026-07-20 기준):** Phase 1~5, Task 1~8의 코드·자동테스트(빌드/`gradlew test`/
> `npm test`/`vitest`)는 전부 완료·커밋됨 — `ragbot-server` HEAD `b1948be`,
> `remote-contrabass-admin` HEAD `44b6d9204`. 남은 것은 각 Phase의 **사용자 검수**(실 브라우저·실
> 로그인·실 Prometheus 확인 — Claude가 대신 할 수 없음)뿐이다. Phase 4 Task 5 Step 7(브라우저 수동
> 확인)도 같은 이유로 미체크 상태다.
>
> **사용자 검수 중 발견된 결함 2건(수정 완료, 미커밋)**:
> 1. **Phase 2 컨텍스트 폴백이 실제로는 동작하지 않음** — `ResourcePrompts.kt`의 QUOTA 추출 프롬프트가
>    옛 설계("프로젝트 불명확하면 되물어준다")를 그대로 갖고 있어, LLM이 스스로 `clarificationNeeded=true`를
>    세팅해버리고 이게 `QuotaResolved` 분기보다 먼저 걸려 서비스 계층의 컨텍스트 폴백(Task 3)에 도달하지
>    못했다. 유닛테스트는 LLM을 mock해서 이 틈을 못 잡았다 — 실 LLM 대상 사용자 검수에서 발견.
>    프롬프트 문구 수정 + few-shot 예시 추가로 해결.
> 2. **Phase 5 위젯이 포털에서 전혀 안 뜸** — `CorsConfig`가 `/api/chat`만 열어놨는데, 위젯 스크립트는
>    `<script type="module">`로 주입된다. module 스크립트는 크로스오리진이면 일반 `<script>`와 달리
>    CORS 헤더 없이는 **브라우저가 로드 자체를 차단**한다(Phase 3 설계 당시 누락). `/chat-widget/**`에도
>    같은 `app.cors.allowed-origins`로 CORS 매핑을 추가해 해결.
>
> **추가 변경 (2026-07-21, 이 계획 범위 밖 — QUOTA 기능 자체 삭제):** 실 Prometheus에 QUOTA 관련
> 메트릭(`openstack_{nova,cinder}_limits_*`)이 아예 수집되고 있지 않음을 확인 → 사용자 결정으로
> **QUOTA target을 코드에서 완전히 제거**(라우팅/추출/프롬프트/`quotaGauge`/`quota_gauge` 위젯 전부).
> **Phase 2("QUOTA 컨텍스트 폴백")는 그 결과 목표를 잃었다** — 아래 Phase 2 체크박스는 삭제된 기능의
> 구현 이력으로만 남긴다. `project` 필드 전달(Phase 1·5의 `ChatRequest`/`ChatCommand`/포털 컨텍스트
> 주입)은 사용자 결정으로 **그대로 유지**한다 — 현재 이 값을 소비하는 target은 없는 예약 필드다.
...
- [x] **Step 3: `ChatRequest`/`ChatCommand`/`ChatController`에 `project` 추가**
...
- [x] **Step 3: `QuotaResolved` nullable화 + `toQuota()` 단순화**
...
- [x] **Step 3-b: `DefaultResourceService.kt`를 컴파일 가능하게 최소 수정 (동작은 그대로 보존)**
...
- [x] **Step 1: 실패하는 테스트 작성 — `DefaultChatServiceTest`**
...
- [x] **Step 3: `ResourceService` 인터페이스 + `DefaultResourceService` 구현**
...
- [x] **Step 4: `DefaultChatService`에서 `command.project` 전달**
...
- [x] **Step 6: `DefaultResourceServiceQuotaTest`에 컨텍스트 폴백 테스트 추가**
...
- [x] **Step 3: `AppProperties.Cors` 추가**
...
- [x] **Step 6: `application.yml`에 설정 항목 추가**
...
- [x] **Step 3: `render/chrome.js` 작성 (`index.html`의 `<template id="cc-chrome">` 마크업을 1:1로 이전)**
...
- [x] **Step 5: `chat-widget.js`가 스스로 마운트하도록 변경**
...
- [x] **Step 6: `index.html`을 한 줄 스크립트로 단순화 (자체 마운트 증명)**
...
- [x] **Step 5: `chat-widget.js`에서 사용 — 매 전송 시점에 재조회**
...
- [x] **Step 3: `useChatWidgetEmbed.ts` 작성**
...
- [x] **Step 1: `onMounted`에 위젯 주입 + 컨텍스트 배선 추가**
...
- [x] **Step 2: 로컬 개발용 `.env`/`.env.localhost`에 `VITE_RAGBOT_CHAT_API_BASE` 추가**
