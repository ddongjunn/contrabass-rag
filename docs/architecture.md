...
> 어떻게 만드는가. 요구사항은 [`requirements.md`](requirements.md), 실행 계획은 [`plan.md`](plan.md).
...
- **회복탄력성 — Resilience4j로 통합**(M4): 아웃바운드 `Retry`(429/5xx 백오프)+`CircuitBreaker`+
  `TimeLimiter`(OpenAI 호출 격리), 인바운드 `RateLimiter`(사용자 빈도 제한).
...
- Slack: `com.slack.api:bolt` + `bolt-socket-mode`(M5, Events API 옵션)
...
├── slack/
│   ├── interfaces/             #   SocketModeRunner(수신·ack), SlackEventHandler
...
- **cache**(겸 질의 로그): **이 앱 소유 테이블**, `JdbcTemplate` 조회/저장. 히트 기준
...
- **slack**: Socket Mode 우선(공개 URL 불필요). 수신 즉시 ack(3초 제약) 후 **비동기**(가상스레드)
...
| 키 | 의미 | 초기값 |
| --- | --- | --- |
| `app.openai.chat-model` | 생성 모델 | `gpt-4o` |
...
| `app.cache.cosine-threshold` | 캐시 히트 기준 | `0.95` |
| `app.guard.max-question-len` / `rate-per-min` | 입력 길이 / 분당 한도 | `1000` / `20` |
...
`SLACK_APP_TOKEN`, DB 접속정보(`SPRING_DATASOURCE_URL`/`DB_*`).
