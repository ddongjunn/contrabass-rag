...
│   ├── application/            #   ChatService(오케스트레이터), ChatCommand(+history)/Result
...
└── common/
    ├── config/                 #   OpenAI/PgVectorStore/Slack/Resilience4j 빈, AppProperties
...
| `app.resource.widgets.project-usage-top-n` | `project_usage_bar` 표시 상한. 테넌트가 많으면 채팅창이 덮인다(실측 27개) | `10` |
