...
object RoutingPrompts {
    ...
    val SYSTEM: String = """
        ...
        - RESOURCE: 실제 인프라를 조회해야 하는 질문. 두 가지를 모두 포함한다.
          ...
          · 리소스의 개수·목록·상태 ("볼륨 몇 개", "인스턴스 목록", "상태 분포")
          · 인프라 헬스·용량 ("죽은 에이전트 있어?", "nova 서비스 정상이야?", "스토리지/IP 얼마나 남았어")
        ...
        - **인프라 리소스(인스턴스·볼륨·스냅샷·네트워크·스토리지·에이전트)를 지목했으면 짧아도 RESOURCE다.**
        ...
        [대화] (없음)
        [질문] 죽은 에이전트 있어?
        => {"route":"RESOURCE","confidence":0.92,"reason":"OpenStack 서비스 에이전트 헬스 조회"}
