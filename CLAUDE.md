...
2. 질문 임베딩은 **요청당 1회만** 계산해 캐시 조회·문서 검색에 **재사용**(재계산 금지).
3. 비싼 **LLM 생성은 캐시 미스 + 검색 성공 경로에서만**.
4. 토큰 낭비 방지: 캐시 히트·검색 실패·**유해 입력(금칙어+Moderation)**을 호출 **전에** 차단.
5. 환각 방지: 검색 근거(top-k)에 기반해서만 답하고 **출처(title/page)** 표기.
6. 소유: **`documents`는 읽기 전용**(테이블 생성·색인 적재는 별도 — `db/init`), **시맨틱 캐시/질의
   로그 테이블은 이 앱이 소유**(DDL·조회/저장).
